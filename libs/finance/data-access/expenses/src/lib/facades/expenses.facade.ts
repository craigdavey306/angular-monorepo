...
import { ExpensesFacadeInterface } from './expenses.facade.interface';
...
export class ExpensesFacade implements ExpensesFacadeInterface {
  ...
  inclVat = toSignal(this.store.select(ExpenseSelectors.selectInclVat), {
    initialValue: false,
  });
  expenses = computed<ExpensesViewModel>(() => {
    const inclVat = this.inclVat();
    return {
      expenses: this.expensesSignal().map((expense) =>
        this.mapExpense(expense, inclVat)
      ),
      inclVat,
      total: this.expensesSignal().reduce((acc, expense) => {
        return (
          acc +
          (inclVat
            ? expense.amount.value * (1 + expense.amount.vatPercentage / 100)
            : expense.amount.value)
        );
      }, 0),
    };
  });
  selectedExpense = toSignal(
    this.store.select(ExpenseSelectors.selectSelectedExpense),
    { initialValue: null }
  );
  ...
  private mapExpense(expense: ExpenseModel, inclVat: boolean) {
    ...
    expenseClone.amount.value = inclVat
      ? expenseClone.amount.value *
        (1 + expenseClone.amount.vatPercentage / 100)
      : expenseClone.amount.value;
    return expenseClone;
  }
}
