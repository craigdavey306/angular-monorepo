...
export class ExpensesEffects {
  ...
  private readonly expensesApi = inject(ExpenseHttpService);
  protected readonly store = inject(Store);

  expensesSignal = toSignal(
    this.store.select(ExpenseSelectors.selectExpenses),
    { initialValue: [] }
  );
  ...
  fetchExpenses$ = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(() =>
        this.expensesApi.get().pipe(
          map((expenses: ExpenseModel[]) =>
            ExpenseActions.fetchExpensesSuccess({ expenses })
          ),
  ...
  fetchExpense$ = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(({ id }) => {
        const expense = this.expensesSignal().find(
          (expense) => expense.id === id
        );
        return expense
          ? of(ExpenseActions.selectExpenseSuccess({ expense }))
          : this.expensesApi.getById(id).pipe(
              map((expense: ExpenseModel) =>
                ExpenseActions.selectExpenseSuccess({ expense })
              ),
  ...
  addExpense$ = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(({ expense }) =>
        this.expensesApi.post(expense).pipe(
          map((expense: ExpenseModel) =>
  ...
  deleteExpense$ = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(({ id }) =>
        this.expensesApi.delete(id).pipe(
          map((expense: ExpenseModel) =>
            ExpenseActions.deleteExpenseSuccess({ id: expense.id as number })
          ),
  ...
  getExpense$ = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(({ id }) => {
        ...
        return expense
          ? of(ExpenseActions.getExpenseSuccess({ expense }))
          : this.expensesApi.getById(id).pipe(
              map((expense: ExpenseModel) =>
                ExpenseActions.getExpenseSuccess({ expense })
              ),
  ...
  updateExpense$ = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(({ expense }) =>
        this.expensesApi.put(expense).pipe(
          map((expense: ExpenseModel) =>
